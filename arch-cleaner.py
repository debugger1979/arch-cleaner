...
def parse():
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="don't delete right away, store and delete on SIGTERM or finish.",
    )
    parser.add_argument(
        "-d",
        "--show-desc",
        default=False,
        action="store_true",
        help="don't ask to show description, show it right away.",
    )
    parser.add_argument(
        "-n",
        "--remove-configs",
        default=False,
        action="store_true",
        help="remove configuration and other normally backupped files.",
    )
    parser.add_argument(
        "--core",
        default=False,
        action="store_true",
        help="suggest packages from core repository as well.",
    )
    parser.add_argument(
...
def getSortedPackageList(includeCore=False):
    ...
    if not includeCore:
        packageList = [
            package for package in packageList if not package.startswith("core")
        ]
    return sorted(
        packageList,
...
def main():
    ...
    try:
        ...
        for package in packages:
            ...
            if choice in ["y", "Y"]:
                ...
                try:
                    ...
                    print(
                        "Unexpected mismatch in package name string:",
                        str(ve),
                        "({})".format(match.groups()),
                    )
                    continue
        ...
        if (len(packagesForDeletion) + len(packagesForDeletionWithConfigs)) > 0:
            try:
                choice = choose(
                    "Delete {} packages now? List them? [y/N/l]".format(
                ...
                if choice in ["y", "Y"]:
                    ...
                    if len(packagesForDeletionWithConfigs) > 0:
                        removePackages(
                            " ".join(packagesForDeletionWithConfigs), configs=True
                        )
            except KeyboardInterrupt:
                print()
        else:
